...
DT::datatable(county_med_age_dt[1:6,], caption = 'Table 3.1: Median age for US counties')
```
...
DT::datatable(county_med_age_young_to_old_dt[1:6,], caption = 'Table 3.2: The youngest counties in the US by median age')
```
...
DT::datatable(county_med_age_old_to_young_dt[1:6,], caption = 'Table 3.3: The oldest counties in the US by median age')
```
...
DT::datatable(county_med_age_gte_50_dt[1:6,], caption = 'Table 3.4: Counties with a median age of 50 or above')
```
...
DT::datatable(county_med_age_dt[1:6,], caption = 'Table 3.5: Separate columns for county and state')
```
...
```{r}
DT::datatable(estimate_long_dt[1:6,], caption = 'Table 3.6: Race and ethnicity in Arizonia')
```
...
DT::datatable(estimate_percent_long_dt[1:6,], caption = 'Table 3.7: Race and ethnicity in Arizonia as percentages')
```
...
DT::datatable(estimate_percent_grouped_dt, caption = 'Table 3.8: Largest group by county in Arizona')
```
...
DT::datatable(estimate_percent_median_dt, caption = 'Table 3.9: Median percentage by group in Arizona')
```
...
DT::datatable(mn_hh_income_dt[1:6,], caption = 'Table 3.10: Table/Group B19001 for counties in Minnesota')
```
...
DT::datatable(mm_hh_income_recode_dt[1:12,], caption = 'Table 3.11: Recoded household income categories')
```
...
DT::datatable(mm_hh_income_recode_grouped_dt[1:6,], caption = 'Table 3.12: Grouped sums by income bands')
```
...
DT::datatable(oglala_lakota_age_2020_dt[1:6,], caption = 'Table 3.13: Age table/group for Oglala Lakota County')
```
...
DT::datatable(oglala_lakota_age_2010_dt[1:6,], caption = 'Table 3.14: Age table/group for Oglala Lakota County, SD (then named Shannon County)')
```
...
DT::datatable(DP02_0068_2019_dt, caption = 'Table 3.15: ACS Data Profile data in 2019')
```
...
DT::datatable(DP02_0068_2018_dt, caption = 'Table 3.16: ACS Data Profile data in 2018')
```
...
DT::datatable(hh_income_vars_dt, caption = 'Table 17a: Variables available that addresses median household income')
```
...
DT::datatable(alaska_hh_incomes_long_dt[1:6,], caption = 'Table 3.17: Comparative income data from the ACS CP tables')
```
...
DT::datatable(vars_B15002_dt, caption = "Table 18a: 'B15002' variables of interest")
```
...
DT::datatable(college_by_year_estimate_long_dt[1:6,], caption = 'Table 3.18: Educational attainment over time for Colorado counties')
```
...
DT::datatable(percent_college_by_year_dt, caption = 'Table 3.19: Percent college by year in Colorado counties')
```
